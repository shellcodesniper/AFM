...
def save_to_file_csv(file_name, data) :
    # with open(file_name,'w',encoding="cp949") as make_file: 
    exists = os.path.exists('data/coin.csv')
    with open('data/coin.csv', 'at') as make_file:
        # title 저장
        if not exists:
          vals = data[0].keys()
          ss = ''
...
TICK_FROM       = '00:00:00'    # HH:mm:ss  (utc 기준임)  or None (00:00:00)
TICK_TO         = None #'00:05:00'    # HH:mm:ss  (utc 기준임)  or None (최근)
